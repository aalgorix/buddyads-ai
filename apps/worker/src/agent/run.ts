...
import { env, getBookCallUrl, getModels, modelsForPlatforms } from '../env';
...
function buildResearchQuestions(
  ...
  const primaryComp = (intake?.competitors || '')
    .split(/[,;]/)[0]
    ?.trim();

  const questions = [
    `What are the best ${product} for ${audience} in ${geo}? Name specific companies and why you would recommend them.`,
    `A buyer is looking for solutions like: ${product}. ${desc}. Which brands would you recommend for ${audience}? Name specific companies.`,
    `Who are the top alternatives to ${brand} (${product})? Consider competitors such as ${comps} and any others you would name first.`,
    primaryComp
      ? `Compare ${brand} vs ${primaryComp} for ${audience} buying ${product}. Who would you recommend and why? Cite sources if you have them.`
      : `How does ${brand} compare with other ${product} options for ${audience}? Who leads, and why?`,
    `Which ${product} would you recommend to ${audience} in ${geo}, and which sources or reviews would you trust? Include URLs if known.`,
    `What should ${audience} evaluate before choosing ${brand} or a similar ${product}? Which vendors typically appear on a shortlist?`,
    ...
    `Where would you send someone who wants independent information about ${brand} and ${product}? Name publications, directories, or review sites.`,
  ];
...
export async function runVisibilityAgent(jobId: string): Promise<void> {
  ...
  const trace: Trace[] = [];
  const platforms = intake?.aiPlatforms || null;
...
async function runFallbackAgent(
  ...
  for (const q of questions) {
    const answers = await queryAllModels({ question: q, brandName: brand, platforms });
...
async function runPlannerAgent(
  ...
  const platforms = intake?.aiPlatforms || null;
  const models = modelsForPlatforms(platforms).join(', ') || getModels().join(', ');
...
- Prefer 6–8 distinct buyer-style research questions covering category, alternatives, comparison, evaluation, and sources.
- When you have crawl data and at least several successful answers, action=finalize_report.
- Never invent crawl data. Do not include markdown.`;
  ...
  for (let step = 1; step <= maxSteps; step++) {
    ...
    if (decision.action === 'query_llms') {
      ...
      const question =
        ...
        fallbacks[Math.min(Math.floor(state.research.length / Math.max(1, modelsForPlatforms(platforms).length)), fallbacks.length - 1)];
      const answers = await queryAllModels({ question, brandName: brand, platforms });
      state.research.push(...answers);
      continue;
    }
    ...
    if (decision.action === 'finalize_report') {
      ...
      if (state.research.length === 0) {
        const brand =
          state.crawl.brandGuess || intake?.companyName || customer.company || customer.name || websiteUrl;
        for (const question of buildResearchQuestions(brand, state.crawl.title, intake)) {
          const answers = await queryAllModels({ question, brandName: brand, platforms });
          state.research.push(...answers);
        }
      }
  ...
  const brand =
    state.crawl?.brandGuess || intake?.companyName || customer.company || customer.name || websiteUrl;
  const asked = new Set(state.research.map((r) => r.question));
  const remaining = buildResearchQuestions(brand, state.crawl?.title || brand, intake).filter((q) => !asked.has(q));
  if (remaining.length && asked.size < 6) {
    await updateStep('research_fill');
    for (const question of remaining.slice(0, Math.max(0, 8 - asked.size))) {
      const answers = await queryAllModels({ question, brandName: brand, platforms });
      state.research.push(...answers);
      trace.push({ step: trace.length + 1, action: 'query_llms', detail: question.slice(0, 120) });
    }
  }
